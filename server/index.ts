...
(async () => {
  ...
  // In development, always use port 5000 to match Replit workflow expectations
  const port = process.env.NODE_ENV === 'development' ? 5000 : parseInt(process.env.PORT || defaultPort, 10);
  server.listen({
    port,
    host: "0.0.0.0",
    reusePort: true,
