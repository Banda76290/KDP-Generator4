...
- **API Call Bug Fix**: Fixed critical API calling pattern in admin system interface (August 2025). Corrected multiple apiRequest() calls that used incorrect parameter order, causing "Unexpected token JSON" errors. All admin functions now use proper signature: apiRequest(url, {method, body}) instead of apiRequest(method, url).
