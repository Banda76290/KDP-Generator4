...
- **Deployment Crash Fix**: Resolved framer-motion production crash (August 2025). Combined server-side exclusion (--external:framer-motion in package.json build script) with client-side exclusion (rollupOptions.external in vite.config.ts) to completely eliminate framer-motion from both bundles. Fixed "TypeError: Cannot read properties of undefined (reading 'join')" that caused deployment failures. Server now starts correctly in production environment.
